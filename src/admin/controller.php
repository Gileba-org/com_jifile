...
class JifileController extends JControllerLegacy {
	...
	function display($cachable = false, $param = array()) {
		...
		Factory::getApplication()->input->def('view','error');
		...
		Factory::getApplication()->input->def('view','config');
				$view = $this->getView('error', 'html');
				parent::display($cachable);
				return false;
			}
		...
		if (!Factory::getApplication()->getRouter()->getVar('view')) {
			...
			Factory::getApplication()->input->def('view','frontpage');
		...
		$state = Factory::getApplication()->getRouter()->getVar('state');
		if($state) {
			switch ($state) {
				case 'sync':
